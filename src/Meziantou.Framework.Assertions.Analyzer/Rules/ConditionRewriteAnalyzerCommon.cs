...
internal static class ConditionRewriteAnalyzerCommon
{
    ...
    internal static bool TryGetEqualsMethodMatch(IInvocationOperation assertInvocation, INamedTypeSymbol assertType, Symbols symbols, out ConditionRewriteMatch match)
    {
        ...
        if (SymbolEqualityComparer.Default.Equals(invocation.TargetMethod, symbols.ObjectStaticEqualsMethod) && invocation.Arguments.Length == 2)
        {
            var expectedOperand = AssertionsAnalyzerHelpers.UnwrapImplicitConversion(invocation.Arguments[0].Value);
            var actualOperand = AssertionsAnalyzerHelpers.UnwrapImplicitConversion(invocation.Arguments[1].Value);
            if (IsSequenceComparedByAssertEqual(expectedOperand.Type, symbols) || IsSequenceComparedByAssertEqual(actualOperand.Type, symbols))
            {
                match = default;
                return false;
            }
        ...
        if (invocation is { Instance: { } instance, Arguments.Length: 1 } && !invocation.TargetMethod.IsStatic)
        {
            ...
            if (IsSequenceComparedByAssertEqual(actualOperand.Type, symbols) || IsSequenceComparedByAssertEqual(expectedOperand.Type, symbols))
            {
                match = default;
                return false;
            }
    ...
    internal static bool TryGetRangeMatch(IInvocationOperation assertInvocation, INamedTypeSymbol assertType, Symbols symbols, out ConditionRewriteMatch match)
    {
        ...
        if (!TryGetBound(andOperation.LeftOperand, symbols, out var firstValue, out var firstBound, out var firstIsLowerBound) ||
            !TryGetBound(andOperation.RightOperand, symbols, out var secondValue, out var secondBound, out var secondIsLowerBound) ||
            firstIsLowerBound == secondIsLowerBound ||
            !SyntaxFactory.AreEquivalent(firstValue.Syntax, secondValue.Syntax))
        {
            match = default;
            return false;
        }

        var lowOperand = firstIsLowerBound ? firstBound : secondBound;
        var highOperand = firstIsLowerBound ? secondBound : firstBound;
        var assertionMethodName = conditionExpectedToBeFalse ? NotInRangeAssertionMethodName : InRangeAssertionMethodName;
        match = new ConditionRewriteMatch(andOperation, assertionMethodName, [firstValue, lowOperand, highOperand]);
        return true;
    }

    private static bool TryGetBound(IOperation operation, Symbols symbols, out IOperation value, out IOperation bound, out bool isLowerBound)
    {
            ...
            IsComparableWithDefaultComparer(binaryOperation, symbols))
        {
            ...
            // 'value >= bound' and 'bound <= value' both express a lower bound
            var leftIsValue = !AssertionArgumentOrderAnalyzerCommon.IsConstantOrCollectionContainingConstant(left) ||
                              AssertionArgumentOrderAnalyzerCommon.IsConstantOrCollectionContainingConstant(right);
            if (leftIsValue)
            {
                value = left;
                bound = right;
                isLowerBound = binaryOperation.OperatorKind == BinaryOperatorKind.GreaterThanOrEqual;
            }
            else
            {
                value = right;
                bound = left;
                isLowerBound = binaryOperation.OperatorKind == BinaryOperatorKind.LessThanOrEqual;
            }
    ...
    /// <summary>
    /// <c>Assert.Equal</c> compares sequences element by element, so rewriting <c>Equals</c> on a sequence type
    /// would change the comparison being performed.
    /// </summary>
    private static bool IsSequenceComparedByAssertEqual(ITypeSymbol? type, Symbols symbols)
    {
        if (type is null)
            return false;
