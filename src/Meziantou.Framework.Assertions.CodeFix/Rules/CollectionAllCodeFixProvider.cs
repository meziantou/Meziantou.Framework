...
[ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(CollectionAllCodeFixProvider))]
public sealed class CollectionAllCodeFixProvider : CodeFixProvider
{
    public override ImmutableArray<string> FixableDiagnosticIds =>
        [RuleIdentifiers.UseCollectionAllDiagnosticId, RuleIdentifiers.UseCollectionDoesNotAllDiagnosticId];
    ...
    public override async Task RegisterCodeFixesAsync(CodeFixContext context)
    {
        ...
        foreach (var diagnostic in context.Diagnostics)
        {
            ...
            if (!TrueFalseConditionMethodSelectionAnalyzerCommon.TryGetCollectionAllMatch(innerInvocation, symbols.Value, conditionExpectedToBeFalse, out var match))
                continue;
