...
[ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(CollectionAnyCodeFixProvider))]
public sealed class CollectionAnyCodeFixProvider : CodeFixProvider
{
    public override ImmutableArray<string> FixableDiagnosticIds =>
        [RuleIdentifiers.UseCollectionAnyContainsDiagnosticId, RuleIdentifiers.UseCollectionAnyDoesNotContainDiagnosticId];
    ...
    public override async Task RegisterCodeFixesAsync(CodeFixContext context)
    {
        ...
        foreach (var diagnostic in context.Diagnostics)
        {
            ...
            if (!TrueFalseConditionMethodSelectionAnalyzerCommon.TryGetCollectionAnyMatch(innerInvocation, symbols.Value, conditionExpectedToBeFalse, out var match))
                continue;
