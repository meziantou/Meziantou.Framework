...
[ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(CollectionContainsCodeFixProvider))]
public sealed class CollectionContainsCodeFixProvider : CodeFixProvider
{
    public override ImmutableArray<string> FixableDiagnosticIds =>
        [RuleIdentifiers.UseCollectionContainsDiagnosticId, RuleIdentifiers.UseCollectionDoesNotContainDiagnosticId];
    ...
    public override async Task RegisterCodeFixesAsync(CodeFixContext context)
    {
        ...
        foreach (var diagnostic in context.Diagnostics)
        {
            ...
            if (!TrueFalseConditionMethodSelectionAnalyzerCommon.TryGetCollectionContainsMatch(innerInvocation, symbols.Value, conditionExpectedToBeFalse, out var match) &&
                !TrueFalseConditionMethodSelectionAnalyzerCommon.TryGetDictionaryContainsKeyMatch(innerInvocation, symbols.Value, conditionExpectedToBeFalse, out match))
            {
                continue;
            }
