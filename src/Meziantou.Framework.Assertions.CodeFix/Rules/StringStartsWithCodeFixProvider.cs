...
[ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(StringStartsWithCodeFixProvider))]
public sealed class StringStartsWithCodeFixProvider : CodeFixProvider
{
    public override ImmutableArray<string> FixableDiagnosticIds =>
        [RuleIdentifiers.UseStringStartsWithDiagnosticId, RuleIdentifiers.UseStringDoesNotStartWithDiagnosticId];
    ...
    public override async Task RegisterCodeFixesAsync(CodeFixContext context)
    {
        ...
        foreach (var diagnostic in context.Diagnostics)
        {
            ...
            if (!TrueFalseConditionMethodSelectionAnalyzerCommon.TryGetStringOperationMatch(innerInvocation, symbols.Value, conditionExpectedToBeFalse, out var match))
                continue;
