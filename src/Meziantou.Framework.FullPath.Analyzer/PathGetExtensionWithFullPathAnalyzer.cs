...
public sealed class PathGetExtensionWithFullPathAnalyzer : DiagnosticAnalyzer
{
    ...
    private static void Analyze(OperationAnalysisContext context, FullPathContext analyzerContext)
    {
        ...
        if (invocationOperation.TargetMethod is not { IsStatic: true, Name: "GetExtension" } targetMethod)
            return;
