...
public sealed class PathGetFileNameWithFullPathAnalyzer : DiagnosticAnalyzer
{
    ...
    private static void Analyze(OperationAnalysisContext context, FullPathContext analyzerContext)
    {
        ...
        if (invocationOperation.TargetMethod is not { IsStatic: true, Name: "GetFileName" } targetMethod)
            return;
