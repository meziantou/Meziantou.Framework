...
[ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(FullPathDivisionWithFullPathRightCodeFixProvider))]
public sealed class FullPathDivisionWithFullPathRightCodeFixProvider : CodeFixProvider
{
    public override ImmutableArray<string> FixableDiagnosticIds => [FullPathAnalyzerCommon.DivisionWithFullPathRightDiagnosticId];
    ...
    public override async Task RegisterCodeFixesAsync(CodeFixContext context)
    {
        ...
        foreach (var diagnostic in context.Diagnostics)
        {
            ...
            if (!TryGetReplacementExpression(semanticModel, expression, fullPathType, context.CancellationToken, out _))
                continue;

            context.RegisterCodeFix(
                CodeAction.Create(
                    title: "Use right FullPath value",
                    createChangedDocument: cancellationToken => ApplyFixAsync(context.Document, expression, fullPathType, cancellationToken),
                    equivalenceKey: GetType().FullName),
                diagnostic);
        }
    }

    private static async Task<Document> ApplyFixAsync(Document document, ExpressionSyntax expressionSyntax, INamedTypeSymbol fullPathType, CancellationToken cancellationToken)
    {
        ...
        if (!TryGetReplacementExpression(semanticModel, expressionSyntax, fullPathType, cancellationToken, out var replacementExpression))
            return document;
