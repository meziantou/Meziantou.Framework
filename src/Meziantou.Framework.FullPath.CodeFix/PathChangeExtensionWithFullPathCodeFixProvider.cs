...
[ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(PathChangeExtensionWithFullPathCodeFixProvider))]
public sealed class PathChangeExtensionWithFullPathCodeFixProvider : CodeFixProvider
{
    public override ImmutableArray<string> FixableDiagnosticIds => [FullPathAnalyzerCommon.PathChangeExtensionWithFullPathDiagnosticId];
        ...
        out ExpressionSyntax replacementExpression)
    {
        if (semanticModel.GetOperation(expressionSyntax, cancellationToken) is IInvocationOperation invocationOperation &&
            invocationOperation.TargetMethod is { IsStatic: true, Name: "ChangeExtension" } targetMethod &&
