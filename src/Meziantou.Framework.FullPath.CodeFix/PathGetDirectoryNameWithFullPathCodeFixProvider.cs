...
[ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(PathGetDirectoryNameWithFullPathCodeFixProvider))]
public sealed class PathGetDirectoryNameWithFullPathCodeFixProvider : CodeFixProvider
{
    public override ImmutableArray<string> FixableDiagnosticIds => [FullPathAnalyzerCommon.PathGetDirectoryNameWithFullPathDiagnosticId];
        ...
        out ExpressionSyntax replacementExpression)
    {
        if (semanticModel.GetOperation(expressionSyntax, cancellationToken) is IInvocationOperation invocationOperation &&
            invocationOperation.TargetMethod is { IsStatic: true, Name: "GetDirectoryName" } targetMethod &&
