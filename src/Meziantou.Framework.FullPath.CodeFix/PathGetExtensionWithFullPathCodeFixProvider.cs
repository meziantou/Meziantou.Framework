...
[ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(PathGetExtensionWithFullPathCodeFixProvider))]
public sealed class PathGetExtensionWithFullPathCodeFixProvider : CodeFixProvider
{
    public override ImmutableArray<string> FixableDiagnosticIds => [FullPathAnalyzerCommon.PathGetExtensionWithFullPathDiagnosticId];
        ...
        out ExpressionSyntax replacementExpression)
    {
        if (semanticModel.GetOperation(expressionSyntax, cancellationToken) is IInvocationOperation invocationOperation &&
            invocationOperation.TargetMethod is { IsStatic: true, Name: "GetExtension" } targetMethod &&
