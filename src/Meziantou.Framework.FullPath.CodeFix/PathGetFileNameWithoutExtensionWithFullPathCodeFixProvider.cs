...
[ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(PathGetFileNameWithoutExtensionWithFullPathCodeFixProvider))]
public sealed class PathGetFileNameWithoutExtensionWithFullPathCodeFixProvider : CodeFixProvider
{
    public override ImmutableArray<string> FixableDiagnosticIds => [FullPathAnalyzerCommon.PathGetFileNameWithoutExtensionWithFullPathDiagnosticId];
    ...
    public override async Task RegisterCodeFixesAsync(CodeFixContext context)
    {
        ...
        foreach (var diagnostic in context.Diagnostics)
        {
            ...
            context.RegisterCodeFix(
                CodeAction.Create(
                    title: "Use FullPath.NameWithoutExtension",
        ...
        out ExpressionSyntax replacementExpression)
    {
        if (semanticModel.GetOperation(expressionSyntax, cancellationToken) is IInvocationOperation invocationOperation &&
            invocationOperation.TargetMethod is { IsStatic: true, Name: "GetFileNameWithoutExtension" } targetMethod &&
