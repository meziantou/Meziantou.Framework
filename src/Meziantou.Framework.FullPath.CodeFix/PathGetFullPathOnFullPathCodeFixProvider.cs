...
[ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(PathGetFullPathOnFullPathCodeFixProvider))]
public sealed class PathGetFullPathOnFullPathCodeFixProvider : CodeFixProvider
{
    public override ImmutableArray<string> FixableDiagnosticIds => [FullPathAnalyzerCommon.PathGetFullPathDiagnosticId];
        ...
        out ExpressionSyntax replacementExpression)
    {
        if (semanticModel.GetOperation(expressionSyntax, cancellationToken) is IInvocationOperation invocationOperation &&
            invocationOperation.TargetMethod is { IsStatic: true, Name: "GetFullPath" } targetMethod &&
