...
[ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(PathGetRelativePathWithFullPathCodeFixProvider))]
public sealed class PathGetRelativePathWithFullPathCodeFixProvider : CodeFixProvider
{
    public override ImmutableArray<string> FixableDiagnosticIds => [FullPathAnalyzerCommon.PathGetRelativePathWithFullPathDiagnosticId];
    ...
    public override async Task RegisterCodeFixesAsync(CodeFixContext context)
    {
        ...
        foreach (var diagnostic in context.Diagnostics)
        {
            ...
            context.RegisterCodeFix(
                CodeAction.Create(
                    title: "Use FullPath.MakePathRelativeTo",
        ...
        out ExpressionSyntax replacementExpression)
    {
        if (semanticModel.GetOperation(expressionSyntax, cancellationToken) is IInvocationOperation invocationOperation &&
            invocationOperation.TargetMethod is { IsStatic: true, Name: "GetRelativePath" } targetMethod &&
            ...
            invocationOperation.Arguments.Length == 2)
        {
            ...
            if (SymbolEqualityComparer.Default.Equals(rootOperation.Type, fullPathType) &&
                ...
                valueOperation.Syntax is ExpressionSyntax valueExpression)
            {
                replacementExpression = SyntaxFactory.InvocationExpression(
                    ...
                    [
                        SyntaxFactory.Argument(rootExpression.WithoutTrivia()),
                    ]));
                return true;
            }
        }
