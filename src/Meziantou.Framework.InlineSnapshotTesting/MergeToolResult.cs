...
public abstract class MergeToolResult : IDisposable
{
    ...
    /// <summary>
    /// Indicates whether <see cref="WaitForExit" /> returns only once the developer is done with the merge. A launcher
    /// that hands the files over to an already running IDE exits right away, while the diff is still open.
    /// </summary>
    internal virtual bool WaitsForMerge => true;
}
