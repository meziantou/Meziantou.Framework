...
{
    public static string BuildText<TNode>(IReadOnlyList<TNode>? nodes, IReadOnlyList<RegexSyntaxToken>? separators)
    ...
    public static int GetFullStart<TNode>(IReadOnlyList<TNode>? nodes, IReadOnlyList<RegexSyntaxToken>? separators)
        where TNode : RegexSyntaxNode
    {
        if (nodes is { Count: > 0 })
