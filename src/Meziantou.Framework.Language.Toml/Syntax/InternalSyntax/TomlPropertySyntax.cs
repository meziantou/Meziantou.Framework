...
internal sealed class TomlPropertySyntax : TomlEntrySyntax
{
    ...
    internal override GreenNode SetDiagnostics(SyntaxDiagnosticInfo[]? diagnostics) => new TomlPropertySyntax(_keyToken, _separatorToken, _valueToken, diagnostics, GetAnnotations());
    internal override GreenNode SetAnnotations(SyntaxAnnotation[]? annotations) => new TomlPropertySyntax(_keyToken, _separatorToken, _valueToken, GetDiagnostics(), annotations);
