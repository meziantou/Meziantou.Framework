...
internal static class XmlSyntaxTreeWriter
{
    public static void Write(Utf8JsonWriter writer, XmlSyntaxTree tree, DumpOptions options)
    {
        writer.WriteStartObject();
        writer.WriteString("language", "xml");
    ...
    // Walked with an explicit stack: the XML parser puts no limit on nesting depth, so recursion here would run the
    // stack out on input that parsed perfectly well. A null entry closes the node whose children were just written.
    private static void WriteNode(Utf8JsonWriter writer, XmlSyntaxNode root, DumpOptions options)
    {
        var stack = new Stack<XmlSyntaxNode?>();
        stack.Push(root);
    ...
    private static void WriteTokens(Utf8JsonWriter writer, IReadOnlyList<XmlSyntaxToken> tokens, DumpOptions options)
    {
    ...
    private static void WriteTrivia(Utf8JsonWriter writer, string propertyName, IReadOnlyList<XmlSyntaxTrivia> trivia, DumpOptions options)
    {
