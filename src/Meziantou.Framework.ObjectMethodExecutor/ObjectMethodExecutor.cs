...
public sealed class ObjectMethodExecutor
{
    ...
    public static ObjectMethodExecutor Create(MethodInfo methodInfo, object?[]? parameterDefaultValues)
    {
        ...
        return new ObjectMethodExecutor(methodInfo, methodInfo.DeclaringType.GetTypeInfo(), parameterDefaultValues);
    }
    ...
    /// <summary>
    /// Executes the configured method on <paramref name="target"/>.
    /// </summary>
    /// <remarks>
    ...
    private static MethodExecutorAsync GetExecutorAsyncFromSync(MethodExecutor executor)
    {
        return delegate (object target, object?[]? parameters)
        {
            ...
            return new ObjectMethodExecutorAwaitable(
                task,
                task => ((Task<object?>)task).GetAwaiter(),
                taskAwaiter => ((TaskAwaiter<object?>)taskAwaiter).IsCompleted,
                taskAwaiter => ((TaskAwaiter<object?>)taskAwaiter).GetResult(),
