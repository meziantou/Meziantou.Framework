...
When using `--file-layout SingleFile`, `--output` can be either a directory (generates `PublicApi.g.cs`) or a file path (uses the provided filename).

```bash
Meziantou.Framework.PublicApiGenerator.Tool \
  --input ./bin/Release/net8.0/MyLibrary.dll \
  --output ./public-api/MyLibrary.PublicApi.cs \
...
Options:
  ...
  --output <output> (REQUIRED)                                   Output path (directory, or file path when --file-layout is SingleFile)
