...
internal static class Css
{
    ...
    [
        ...
        "feOffset","feSpecularLighting","feTile","feTurbulence","linearGradient",
        "radialGradient","stop","circle","ellipse","image","line","path","polygon",
        "polyline","rect","text","use","textPath","tspan","foreignObject","clipPath",
    ];

    private static readonly string[] MediaFeatures = SortDescending(
    [
    ...
    private static readonly string[] PseudoClasses = SortDescending(
    [
    ...
    private static readonly string[] PseudoElements = SortDescending(
    [
    ...
    private static readonly string[] Units = SortDescending(
    [
    ...
    private static readonly string[] Attributes = SortDescending(
    [
        ...
        "font-optical-sizing","font-palette","font-size","font-size-adjust","font-smooth",
        "font-smoothing","font-stretch","font-style","font-synthesis","font-synthesis-position",
        ...
        "height","hyphenate-character","hyphenate-limit-chars","hyphens","icon","image-orientation",
        "image-rendering","image-resolution","ime-mode","initial-letter","initial-letter-align",
        "inline-size","inset","inset-area","inset-block","inset-block-end","inset-block-start",
        "inset-inline","inset-inline-end","inset-inline-start","isolation","justify-content",
        "justify-items","justify-self","kerning","left","letter-spacing","lighting-color",
        "line-break","line-height","line-height-step","list-style","list-style-image",
        ...
        "margin-inline-start","margin-left","margin-right","margin-top","margin-trim","marker",
        "marker-end","marker-mid","marker-start","marks","mask","mask-border","mask-border-mode",
        ...
        "mask-position","mask-repeat","mask-size","mask-type","masonry-auto-flow","math-depth",
        "math-shift","math-style","max-block-size","max-height","max-inline-size","max-width",
        "min-block-size","min-height","min-inline-size","min-width","mix-blend-mode","nav-down",
        "nav-index","nav-left","nav-right","nav-up","none","normal","object-fit",
        ...
        "overflow-block","overflow-clip-margin","overflow-inline","overflow-wrap","overflow-x",
        "overflow-y","overlay","overscroll-behavior","overscroll-behavior-block",
        ...
        "stroke-opacity","stroke-width","tab-size","table-layout","text-align","text-align-all",
        ...
        "transform-style","transition","transition-behavior","transition-delay",
        "transition-duration","transition-property","transition-timing-function","translate",
        "unicode-bidi","user-modify","user-select","vector-effect","vertical-align",
    ...
    private static string[] SortDescending(IEnumerable<string> source)
    {
    ...
    {
        var important = new Mode { Scope = "meta", Begin = "!important" };
        var hexColor = new Mode { Scope = "number", Begin = @"#(([0-9a-fA-F]{3,4})|(([0-9a-fA-F]{2}){3,4}))\b" };
        var functionDispatch = new Mode { Scope = "built_in", Begin = @"[\w-]+(?=\()" };
        var attributeSelector = new Mode
        {
            Scope = "selector-attr",
            Begin = @"\[",
            End = @"\]",
            Illegal = "$",
            Contains = [CommonModes.AposStringMode, CommonModes.QuoteStringMode],
        };
        var cssNumberMode = new Mode
        {
            Scope = "number",
            Begin = CommonModes.NumberRe + @"(" + string.Join('|', Units) + @")?",
        };
        var cssVariable = new Mode { Scope = "attr", Begin = @"--[A-Za-z_][A-Za-z0-9_-]*" };

        var vendorPrefix = new Mode { Begin = @"-(webkit|moz|ms|o)-(?=[a-z])" };
        const string AtModifiers = "and or not only";
        const string AtPropertyRe = @"@-?\w[\w]*(-\w+)*";
        const string IdentRe = @"[a-zA-Z-][a-zA-Z0-9_-]*";

        var selectorPseudo = new Mode
        {
            Scope = "selector-pseudo",
            Variants =
            [
                new Mode { Begin = ":(" + string.Join('|', PseudoClasses) + ")" },
                new Mode { Begin = ":(:)?(" + string.Join('|', PseudoElements) + ")" },
            ],
        };

        var urlContent = new Mode
        {
            Scope = "string",
            Begin = "[^)]",
            EndsWithParent = true,
        ...
        var urlMode = new Mode
        {
            Begin = @"(url|data-uri)\(",
            End = @"\)",
            Keywords = Keywords.FromMap(new Dictionary<string, string>(StringComparer.Ordinal) { ["built_in"] = "url data-uri" }),
            Contains =
            [
        ...
        var atFeatureKeyword = new Mode { Scope = "keyword", Begin = AtPropertyRe };
        var atFeatureBody = new Mode
        {
            Begin = @"\s",
            EndsWithParent = true,
            ExcludeEnd = true,
            ...
            {
                ["keyword"] = AtModifiers,
                ["attribute"] = string.Join(' ', MediaFeatures),
            }),
            KeywordPattern = "[a-z-]+",
            Contains =
            [
                new() { Scope = "attribute", Begin = "[a-z-]+(?=:)" },
                CommonModes.AposStringMode,
                CommonModes.QuoteStringMode,
                cssNumberMode,
            ],
        ...
        var atRule = new Mode
        {
            ...
            End = "[{;]",
            Illegal = ":",
            Contains = [atFeatureKeyword, atFeatureBody],
        };
        ...
        return new Mode
        {
            ...
            Illegal = @"[=|'\$]",
            Keywords = Keywords.FromMap(new Dictionary<string, string>(StringComparer.Ordinal) { ["keyframePosition"] = "from to" }),
            ...
            [
                ...
                vendorPrefix,
                cssNumberMode,
                new() { Scope = "selector-id", Begin = "#[A-Za-z0-9_-]+" },
                new() { Scope = "selector-class", Begin = @"\." + IdentRe },
                attributeSelector,
                selectorPseudo,
                cssVariable,
                new() { Scope = "attribute", Begin = @"\b(" + string.Join('|', Attributes) + @")\b" },
                attributeValue,
                atRule,
                new() { Scope = "selector-tag", Begin = @"\b(" + string.Join('|', HtmlTags.Concat(SvgTags)) + @")\b" },
            ],
        };
    }
}
