...
The initial SQL text support is intentionally small: one `SELECT` statement with `FROM`, `INNER JOIN`, `WHERE` comparisons combined with `AND`/`OR`, `IS NULL`/`IS NOT NULL`, `IN`/`NOT IN` (value lists and simple subqueries), SQL parameters, `TOP`, `SELECT *` for single-table queries, selected columns with aliases, `ORDER BY` (including `OFFSET/FETCH`), single-column `GROUP BY`, `HAVING`, grouped aggregates (`COUNT(*)`, `SUM`, `MIN`, `MAX`, `AVG`), and `ORDER BY` with `GROUP BY`.

Main unsupported SQL features include `LIKE`, arithmetic expressions, `DISTINCT`, `UNION`, subqueries outside `IN`, correlated subqueries, multi-column `GROUP BY`, non-aggregate functions, DML (`INSERT`/`UPDATE`/`DELETE`), and multiple statements/result sets.
