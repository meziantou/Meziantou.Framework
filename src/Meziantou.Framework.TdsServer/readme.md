...
The initial SQL text support is intentionally small: one `SELECT` statement with optional non-recursive CTEs (`WITH ... AS (...)`), `FROM`, `INNER JOIN`, `WHERE` comparisons combined with `AND`/`OR`, `IS NULL`/`IS NOT NULL`, `IN`/`NOT IN` (value lists and simple subqueries), SQL parameters, `TOP`, `DISTINCT`, `SELECT *` for single-table queries, selected columns with aliases, `ORDER BY` (including `OFFSET/FETCH`), `UNION`/`UNION ALL`, multi-column `GROUP BY`, `HAVING`, grouped aggregates (`COUNT(*)`, `SUM`, `MIN`, `MAX`, `AVG`), scalar arithmetic operators (`+`, `-`, `*`, `/`, `%`), string concatenation (`+`, `CONCAT`), and scalar functions (`UPPER`, `LOWER`, `LEN`).
...
Main unsupported SQL features include `LIKE`, most scalar functions beyond `UPPER`/`LOWER`/`LEN`/`CONCAT`, recursive CTEs, CTE column lists, subqueries outside `IN`, correlated subqueries, DML (`INSERT`/`UPDATE`/`DELETE`), and multiple statements/result sets.
