...
The initial SQL text support is intentionally small: one `SELECT` statement with `FROM`, `INNER JOIN`, `WHERE` comparisons combined with `AND`/`OR`, `IS NULL`/`IS NOT NULL`, `IN`/`NOT IN` (value lists and simple subqueries), SQL parameters, `TOP`, `DISTINCT`, `SELECT *` for single-table queries, selected columns with aliases, `ORDER BY` (including `OFFSET/FETCH`), `UNION`/`UNION ALL`, multi-column `GROUP BY`, `HAVING`, grouped aggregates (`COUNT(*)`, `SUM`, `MIN`, `MAX`, `AVG`), and `ORDER BY` with `GROUP BY`.

Main unsupported SQL features include `LIKE`, arithmetic expressions, subqueries outside `IN`, correlated subqueries, non-aggregate functions, DML (`INSERT`/`UPDATE`/`DELETE`), and multiple statements/result sets.
