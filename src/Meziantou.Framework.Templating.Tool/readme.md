# Meziantou.Framework.Templating.Tool

<!-- help -->
## Help

```
Description:
  Render a template file using Meziantou.Framework.Templating

Usage:
  meziantou.template [options]

Options:
  --input <input> (REQUIRED)                                 Path to the input template file
  --output <output>                                          Path to the output file. If omitted, writes to stdout
  --start-code-block-delimiter <start-code-block-delimiter>  Delimiter that marks the start of a code block [default: <%]
  --end-code-block-delimiter <end-code-block-delimiter>      Delimiter that marks the end of a code block [default: %>]
  -?, -h, --help                                             Show help and usage information
  --version                                                  Show version information
```
<!-- help -->