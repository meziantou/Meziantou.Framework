...
public sealed class LanguageToolTests(ITestOutputHelper testOutputHelper)
{
    ...
    public async Task Dump_EverySpanSlicesBackToItsOwnText(string language, string text)
    {
        var console = new ConsoleHelper(testOutputHelper);
    ...
    [Theory]
    [InlineData("--no-tokens", "SyntaxToken")]
    [InlineData("--no-tokens", "trailingTrivia")]
    [InlineData("--no-trivia", "trailingTrivia")]
    ...
    /// <summary>
    /// <c>text</c> is a node's full text, so it is <c>fullSpan</c> that slices back to it: <c>span</c> leaves out the
    /// leading and trailing trivia and sits inside <c>fullSpan</c>. A token's <c>text</c> excludes its trivia, so it is
    /// <c>span</c> that slices back to that one.
    /// </summary>
    private static void AssertSpansMatchText(JsonElement nodeOrToken, string text)
    {
        var isToken = nodeOrToken.GetProperty("type").GetString() == "SyntaxToken";

        var fullSpan = nodeOrToken.GetProperty("fullSpan");
        var fullStart = fullSpan.GetProperty("start").GetInt32();
        var fullLength = fullSpan.GetProperty("length").GetInt32();

        var span = nodeOrToken.GetProperty("span");
        ...
        var own = nodeOrToken.GetProperty("text").GetString();
        Assert.Equal(own, isToken ? text.Substring(start, length) : text.Substring(fullStart, fullLength));

        if (nodeOrToken.TryGetProperty("children", out var children))
        {
