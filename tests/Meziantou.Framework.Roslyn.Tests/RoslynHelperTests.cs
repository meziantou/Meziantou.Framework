...
#pragma warning restore CS0618
    ...
    [Fact]
    public void IsGeneratedCodeSymbol_ReturnsTrueWhenTheSymbolHasTheAttribute()
    {
        var compilation = CreateCompilation("""
        ...
        Assert.True(GetRequiredType(compilation, "Sample").IsGeneratedCodeSymbol(compilation));
    }

    [Fact]
    public void IsGeneratedCodeSymbol_ReturnsTrueWhenAContainingSymbolHasTheAttribute()
    {
        var compilation = CreateCompilation("""
            [System.CodeDom.Compiler.GeneratedCode("Tool", "1.0")]
            public class Sample
        ...
        Assert.True(GetRequiredMethod(GetRequiredType(compilation, "Sample"), "Method").IsGeneratedCodeSymbol(compilation));
    }

    [Fact]
    public void IsGeneratedCodeSymbol_ReturnsFalseWhenNoSymbolHasTheAttribute()
    {
        var compilation = CreateCompilation("""
            public class Sample
            {
        ...
        Assert.False(GetRequiredMethod(GetRequiredType(compilation, "Sample"), "Method").IsGeneratedCodeSymbol(compilation));
    }

    [Fact]
    public void IsGeneratedCodeSymbol_ReturnsFalseForATypeDeclaredInSeveralFiles()
    {
        var compilation = CreateCompilation("""
            [System.CodeDom.Compiler.GeneratedCode("Tool", "1.0")]
            public partial class Sample;
            """).AddSyntaxTrees(CSharpSyntaxTree.ParseText("public partial class Sample;", DefaultParseOptions, path: "Other.cs"));

        Assert.False(GetRequiredType(compilation, "Sample").IsGeneratedCodeSymbol(compilation));
    }

    [Fact]
    public void IsGeneratedCodeSymbol_ReturnsFalseForTheMembersOfATypeDeclaredInSeveralFiles()
    {
        var compilation = CreateCompilation("""
            [System.CodeDom.Compiler.GeneratedCode("Tool", "1.0")]
            public partial class Sample
    ...
    [Fact]
    public void IsGeneratedCodeSymbol_ReturnsTrueWhenTheAssemblyHasTheAttribute()
    {
        var compilation = CreateCompilation("""
    ...
    [Fact]
    public void IsGeneratedCodeSymbol_IgnoresNamespacesDeclaredInSeveralFiles()
    {
        // The walk only reaches the assembly when the namespace does not stop it because it is declared twice
        var compilation = CreateCompilation("""
            [assembly: System.CodeDom.Compiler.GeneratedCode("Tool", "1.0")]
            ...
            public class Other;
            """, DefaultParseOptions, path: "Other.cs"));

        Assert.True(GetRequiredType(compilation, "Demo.Sample").IsGeneratedCodeSymbol(compilation));
    }

    [Fact]
    public void IsGeneratedCodeSymbol_ReturnsFalseWhenTheAttributeTypeIsNotResolved()
    {
        var compilation = CreateCompilation("""
            [System.CodeDom.Compiler.GeneratedCode("Tool", "1.0")]
            public class Sample;
        ...
        Assert.False(GetRequiredType(compilation, "Sample").IsGeneratedCodeSymbol(generatedCodeAttribute: null));
    }

    [Fact]
    public void IsGeneratedCode_ReturnsTrueForTheTreeOfASourceGeneratorWhenTheHintNameIndicatesGeneratedCode()
    {
        var compilation = CreateCompilation("public class Sample;");

        var (_, generatedTree) = RunGenerator(compilation, "Generated.g.cs", "public class Generated;");

        Assert.True(generatedTree.IsGeneratedCode(default));
    }

    [Fact]
    public void IsGeneratedCode_ReturnsTrueForTheTreeOfASourceGeneratorThatEmitsAnAutoGeneratedComment()
    {
        var compilation = CreateCompilation("public class Sample;");

        var (_, generatedTree) = RunGenerator(compilation, "Generated.cs", """
            // <auto-generated/>
    ...
    [Fact]
    public void IsGeneratedCode_ReturnsFalseForTheTreeOfASourceGeneratorThatFollowsNeitherConvention()
    {
        // Roslyn does not flag the trees it gets from a source generator, so a generator whose hint name has no
        // well-known suffix and whose output has no <auto-generated> comment is not detected by the heuristics
        var compilation = CreateCompilation("public class Sample;");

        var (_, generatedTree) = RunGenerator(compilation, "Generated.cs", "public class Generated;");

        Assert.False(generatedTree.IsGeneratedCode(default));
    }

    [Fact]
    public void IsGeneratedCodeSymbol_ReturnsTrueForTheSymbolsOfASourceGeneratorThatEmitsTheAttribute()
    {
        // The symbol check catches what the file check above misses, which is why both are needed
        var compilation = CreateCompilation("public class Sample;");

        var (generatedCompilation, generatedTree) = RunGenerator(compilation, "Generated.cs", """
            [System.CodeDom.Compiler.GeneratedCode("Tool", "1.0")]
            public class Generated;
            """);

        Assert.False(generatedTree.IsGeneratedCode(default));
        Assert.True(GetRequiredType(generatedCompilation, "Generated").IsGeneratedCodeSymbol(generatedCompilation));
    }
    ...
    private static (Compilation Compilation, SyntaxTree GeneratedTree) RunGenerator(Compilation compilation, string hintName, string source)
    {
        var driver = CSharpGeneratorDriver.Create([new SampleGenerator(hintName, source).AsSourceGenerator()], parseOptions: DefaultParseOptions);

        driver.RunGeneratorsAndUpdateCompilation(compilation, out var generatedCompilation, out var diagnostics, TestContext.Current.CancellationToken);
        Assert.Empty(diagnostics);

        var generatedTree = Assert.Single(generatedCompilation.SyntaxTrees.Except(compilation.SyntaxTrees));

        return (generatedCompilation, generatedTree);
    }
...
#pragma warning disable RS1041 // This compiler extension should not be implemented in an assembly with target framework
    // The [Generator] attribute is not needed, and would report RS1041 in this assembly: the driver takes the instance
    private sealed class SampleGenerator(string hintName, string source) : IIncrementalGenerator
    {
