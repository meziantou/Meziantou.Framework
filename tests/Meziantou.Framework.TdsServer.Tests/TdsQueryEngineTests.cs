...
public sealed class TdsQueryEngineTests
{
    ...
    [Fact]
    public async Task SqlClient_QueryEngine_MoreProjectionShapesThanTheCacheRetains_KeepBeingServed()
    {
        ...
        using var server = new TdsServer(
            options,
            (context, cancellationToken) => ValueTask.FromResult(TdsAuthenticationResult.Success("master")),
            TdsQueryEngine.CreateQueryHandler(CreateQueryEngineOptions()));
        ...
        for (var index = 0; index < ProjectionShapeChurnCount; index++)
        {
            ...
            Assert.Equal(alias, await ReadFirstColumnNameAsync(connection, $"SELECT Id AS {alias} FROM customers"));
        }
        ...
        Assert.Equal(evictedAlias, await ReadFirstColumnNameAsync(connection, $"SELECT Id AS {evictedAlias} FROM customers"));

        // The cache is process-wide: a handler created after the churn used to inherit an exhausted one.
        var laterOptions = new TdsServerOptions();
        laterOptions.AddTcpListener(0, IPAddress.Loopback);

        using var laterServer = new TdsServer(
            laterOptions,
            (context, cancellationToken) => ValueTask.FromResult(TdsAuthenticationResult.Success("master")),
            TdsQueryEngine.CreateQueryHandler(CreateQueryEngineOptions()));

        await laterServer.StartAsync();
        var laterPort = Assert.Single(laterServer.Ports);

        await using var laterConnection = new SqlConnection(CreateConnectionString(laterPort));
        await laterConnection.OpenAsync();

        var freshAlias = $"{aliasPrefix}_fresh";
        Assert.Equal(freshAlias, await ReadFirstColumnNameAsync(laterConnection, $"SELECT Id AS {freshAlias} FROM customers"));
    }

    [Fact]
    public async Task SqlClient_QueryEngine_ProjectionTypesLeavingTheCache_AreReclaimed()
    {
        ...
        Assert.Equal(trackedAlias, await ReadFirstColumnNameAsync(connection, $"SELECT Id AS {trackedAlias} FROM customers"));
    ...
    [SuppressMessage("Security", "CA2100:Review SQL queries for security vulnerabilities", Justification = "The SQL query is generated within the test and not user-controlled.")]
    private static async Task<string> ReadFirstColumnNameAsync(SqlConnection connection, string commandText)
    {
        await using var command = connection.CreateCommand();
