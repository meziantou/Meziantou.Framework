...
var unicodeDataPath = outputPath / "Resources" / "UnicodeData.bin.br";
...
async Task WriteUnicodeCharacterInfosFile(List<(int Start, int End, string Name)> blockRanges)
{
    ...
    var content = string.Create(CultureInfo.InvariantCulture, $$"""
        ...
        // Binary resource: UnicodeData.bin.br
        // Statistics:
...
static byte[] BuildUnicodeDataBinary(List<UnicodeDataEntry> entries)
{
    ...
    {
        using (var stream = new System.IO.Compression.BrotliStream(compressed, System.IO.Compression.CompressionLevel.SmallestSize, leaveOpen: true))
        {
